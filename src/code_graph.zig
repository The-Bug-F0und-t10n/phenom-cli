...
fn renderHtml(allocator: std.mem.Allocator, out: *std.ArrayList(u8), graph: Graph) !void {
    ...
    try out.appendSlice(allocator,
        ...
        \\function place(view){const left=220,right=90,top=58,head=36,rowGap=46,laneGap=18,nodeMinX=52,w=Math.max(1200,svg.clientWidth*2.4),lanes=new Map(),groups=new Map();for(const n of view.nodes){if(!groups.has(n.path))groups.set(n.path,[]);groups.get(n.path).push(n)}let y=top;for(const path of [...groups.keys()].sort()){const group=groups.get(path).sort((a,b)=>a.line-b.line||a.name.localeCompare(b.name));const rowRight=[];for(const n of group){const max=fileMaxLine.get(n.path)||Math.max(1,n.endLine||n.line||1);const t=Math.max(0,Math.min(1,(n.line||1)/max));const r=Math.min(18,5+n.degree);n._x=left+t*(w-left-right);let row=0;while(rowRight[row]!=null&&n._x-rowRight[row]<nodeMinX+r)row++;rowRight[row]=n._x+r;n._row=row}const rows=Math.max(1,rowRight.length),h=head+rows*rowGap;lanes.set(path,{path,y,h,rows});for(const n of group)n._y=y+head+n._row*rowGap;y+=h+laneGap}return{lanes,w,h:y+30,left,right}}
...
test "caveman graph writes standalone local html" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, html, "rowGap=46") != null);
    try std.testing.expect(std.mem.indexOf(u8, html, "nodeMinX=52") != null);
    try std.testing.expect(std.mem.indexOf(u8, html, "Rows separate close symbols") != null);
}
