...
Status: implemented-verified-offline-real-pending.
...
- `src/audit.zig` define taxonomia `model_protocol`, `tool_contract`, `tool_runtime`, `infrastructure`, `insufficient_evidence` e `validation_failed`.
- SQLite continua usando a tabela append-only `events`; novos eventos `turn_phase` e `turn_error` evitam migration arriscada e preservam replay.
- `renderTurnReplay` gera replay textual deterministico e limitado por linha, sem despejar payload bruto grande.
- `src/main.zig` registra falhas tipadas em backend/modelo, tool gate, envelope, tool runtime, validacao e finalizacao bloqueada.

Validacao executada:

- `zig test src/audit.zig -lc -lsqlite3` -> passou.
- `zig test src/main.zig -lc -lsqlite3` -> passou.
- `zig build test` -> passou.

Risco residual:

- Smoke real SQLite foi implementado em `real-alignment-smoke`, mas nao foi executado nesta etapa por depender de backend local ativo.
...
Status: implemented-verified-real-partial.
...
- `tools/check_product_guardrails.sh` agora roda `audit`, `http`, `render` e `main`, alem dos guardrails anteriores.
- `tools/check_alignment_tasks.sh` foi criado no diretorio atual e valida `TASKS.md`/`alinhamento.md` locais para as tasks criticas.
- `tools/check_real_alignment.sh` cria smoke real opt-in que exige backend real, marcador visivel e eventos SQLite: `model_backend`, fases `intent/final`, `expectation_passed` e ausencia de `assistant_offline_stub`.
- `tools/check_real_patch.sh` cria projeto temporario, exige `collect_evidence`, `apply_patch`, `validate_syntax`, marcador visivel e eventos SQLite correspondentes; em sucesso limpa o workdir, em falha preserva automaticamente o path para diagnostico.
- `build.zig` adiciona `real-alignment-smoke` com opcoes de backend/host/model/session/prompt/expect.
- `src/product_guardrails.zig` cobre A2/A6/A7/A8/A11 com evidencias concretas e checks offline.

Validacao executada:

- `sh tools/check_product_guardrails.sh` -> passou.
- `sh tools/check_real_patch.sh ./zig-out/bin/phenom llamacpp 192.168.1.122:11434 phenom:latest real-patch-20260721k` -> passou; aplicou patch real, validou `src/math.zig` e checou SQLite.
- `zig build test` -> passou.

Risco residual:

- A suite real auditavel cobre patch end-to-end e runtime HTTP, mas ainda nao cobre toda a matriz TS: git/tools amplas, news, browser DOM real, mutation stale negativa, validacao falha e multiplos backends reais.
...
- `src/render.zig` ganhou fixture visual ampla em 40/80/120/180 colunas cobrindo user multiline, thinking, markdown, code block, tool output truncado, diff, tool failure e `Worked for`.
- `src/main.zig` reforca restore SQLite com replay separado: eventos internos `turn_phase`/`turn_error` aparecem no replay, mas nao poluem o transcript visual restaurado.

Validacao executada:

- `zig test src/render.zig` -> passou.
- `zig test src/main.zig -lc -lsqlite3` -> passou.
- `sh tools/check_product_guardrails.sh` -> passou.

Risco residual:

- Snapshot PTY real/interativo com resize/statusbar animada nao foi executado nesta etapa; a prova atual cobre renderer append-only e restore offline.
...
- `src/audit.zig` define `OperationalPhase` e validador de transicoes para `intent -> contract -> evidence -> mutation -> validation -> final`.
- `src/main.zig` registra fase no inicio do turno, no dispatch de tools e no fechamento final.
- O loop existente foi preservado: contratos continuam model-driven, sem portar uma segunda maquina de estados paralela.
- Finalizacao bloqueada por contrato agora registra `turn_error class=insufficient_evidence`.
- Mutacao/validacao entram no loop por contrato: `set_operational_contract` seleciona `mutate_file`; `collect_evidence` entrega micro-contexto; `apply_patch` exige `contextId` fresco; `validate_syntax` registra evidencia diagnostica antes do final.
- Reparos de protocolo para mutacao/validacao ficam estreitos: so ativam por contrato/ferramenta explicita, nao por inferencia ampla de linguagem natural.

Validacao executada:

- `zig test src/audit.zig -lc -lsqlite3` -> passou.
- `zig test src/main.zig -lc -lsqlite3` -> passou; 334 testes.
- `sh tools/check_real_patch.sh ./zig-out/bin/phenom llamacpp 192.168.1.122:11434 phenom:latest real-patch-20260721k` -> passou.
- `zig build test` -> passou.

Risco residual:

- Loop operacional real ja cobre patch simples end-to-end, mas ainda nao cobre git/tools amplas do TS, validadores por linguagem alem de Zig sintatico, browser DOM ou run-tests completo.
...
- `src/http.zig` adiciona `BackendFailureKind`: `connect`, `http_status`, `protocol_parse`, `model_empty`, `model_think_only`, `stream_cancelled`, `stream_read`, `stream_write`, `unknown`.
- `src/main.zig` audita `model_backend` por turno com backend, endpoint, modelo e thinking mode.
- `src/main.zig` audita `backend_metadata` por turno com tokenizer disponivel/indisponivel, `schema_baseline_tokens` e `context_window` quando o backend expor.
- Falhas de stream inicial, search-plan e follow-up do tool loop registram `turn_error` com classe objetiva.
- Resposta vazia/think-only e `expect_contains` falho entram como erro tipado, nao como sucesso silencioso.
- `inspect_runtime` executa HTTP GET limitado para alvos `http://` e registra `runtime_inspection`; indisponibilidade DOM/browser fica explicita em vez de fingida.

Validacao executada:

- `zig test src/http.zig -lc` -> passou.
- `zig test src/main.zig -lc -lsqlite3` -> passou; 334 testes.
- Smoke real runtime `real-runtime-20260721f` -> passou; registrou `contract_selected inspect_runtime`, `tool_start inspect_runtime`, `runtime_inspection status=415 server=llama.cpp` e `expectation_passed PHENOM_RUNTIME_OK`.
- `sh tools/check_product_guardrails.sh` -> passou.

Risco residual:

- `stream_timeout`, DNS por endereco, native tool capability real, DOM/browser automation e matriz real multi-backend seguem fora desta etapa. `n_ctx/schema baseline` ja sao auditados quando o backend fornece; quando nao fornece, ficam `unavailable`, sem estimativa falsa.
