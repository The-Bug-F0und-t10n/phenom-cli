...
command -v python3 >/dev/null 2>&1 || {
  printf 'parse-error-repair-flow: python3 is required for scripted backend\n' >&2
  exit 1
}
...
def send(conn, status, content_type, body):
    ...
    conn.sendall(
        f"HTTP/1.1 {status}\r\nContent-Type: {content_type}\r\nServer: phenom-parse-error-repair-smoke\r\nContent-Length: {len(raw)}\r\nConnection: close\r\n\r\n".encode("ascii") + raw
    )
...
responses = [
    ...
    f"resposta final\n</think>\nFormato corrigido. Resposta direta final. {expect}",
]
while True:
    conn, _ = sock.accept()
    with conn:
